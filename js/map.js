...
var createAd = function (index) {
  var xCoord = window.data.getRandFromRange(300, 900);
  var yCoord = window.data.getRandFromRange(100, 500);
  return {
    author: {
    ...
    offer: {
      ...
      price: window.data.getRandFromRange(1000, 1000000),
      type: AdParams.TYPE[window.data.getRandFromRange(0, AdParams.TYPE.length)],
      rooms: window.data.getRandFromRange(1, 5),
      guests: window.data.getRandFromRange(1, 20),
      checkin: AdParams.CHECK_IN_OUT_TIME[window.data.getRandFromRange(0, AdParams.CHECK_IN_OUT_TIME.length - 1)],
      checkout: AdParams.CHECK_IN_OUT_TIME[window.data.getRandFromRange(0, AdParams.CHECK_IN_OUT_TIME.length - 1)],
      features: window.data.chooseFeatures(window.data.getRandFromRange(0, AdParams.FEATURES.length), AdParams.FEATURES, true),
      description: '',
      photos: []
    },
    location: {
...
var mapActivate = function () {
  ...
  mapPinsListElement.appendChild(createFragment(window.pin.renderPin, advertismentArray));
};
